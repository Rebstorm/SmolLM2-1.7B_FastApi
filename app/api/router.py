...
async def generate(
    ...
    max_new_tokens: Optional[int] = Query(None, description="Maximum new tokens to generate"),
    ...
    if stream:
        return StreamingResponse(llm.stream_generate(query, max_new_tokens=max_new_tokens), media_type="text/plain")
...
async def update_config(
    ...
    llm.update_config(system_prompt=config.system_prompt, max_new_tokens=config.max_new_tokens)
    return {
        "status": "success", 
        "config": {
